...
public class ManagerController extends ModelController {
     ...
     * @param id PathVariable manager's id
    ...
    public ResponseEntity<?> getManager(@PathVariable Integer id) throws Exception {
        return new ResponseEntity<>(managerService.getById(id), new HttpHeaders(), HttpStatus.OK);
    }

    /**
     * GET request to get the list of manager entities
     * @return Returns ResponseEntity with code 200 if successful, 404 otherwise
     * */
    @GetMapping({"/get/{id}"})
    public ResponseEntity<?> getManager() throws Exception {
        return new ResponseEntity<>(managerService.getList(), new HttpHeaders(), HttpStatus.OK);
    }
